...
### Ranking Extension (#2/#3)
- Maintain an ordered list of top picks per slot and defensive class, not just #1:
  - For each armour slot (`Helmet`, `BodyArmour`, `Gloves`, `Boots`, `Shield`) and each defence type group (e.g., `armour`, `evasion`, `energy_shield`, hybrid classes, `ward`), store the top 3 base `refName`s in ranked order, as curated from the BiS table.
- Proposed structure:
  - `BIS_RANKINGS: Record<ItemCategory, Record<DefenseType, string[]>>` where each array is `[#1, #2, #3]`.
  - Keep `BIS_BASES` as a flattened `Set<string>` for quick “is BiS at all” checks; use `BIS_RANKINGS` for rank calculation.
- Helper functions:
  - `isBisBase(item): boolean` — unchanged quick check via `BIS_BASES`.
  - `getBisRank(item): 1 | 2 | 3 | null` — returns rank based on `item.category`, `DefenseType` group(s) the base belongs to, and the position in `BIS_RANKINGS`.
  - If a base appears in multiple defence groups, prefer the smallest rank (best) and optionally expose the matched group for tooltip.
...
- Price Check header label (`renderer/src/web/price-check/filters/FilterName.vue:35`):
  ...
  - With ranking: append ` (BiS #X)` or show a pill `BiS #X` where `X ∈ {1,2,3}` from `getBisRank(item)`.
  - Optional tooltip includes the defence type group (e.g., “Highest Armour”, “Highest ES”, hybrid class) matched for the rank.

## Internationalization
- Add new key in locales: `item.best_in_slot_base` → "Best-in-slot base".
- Matching relies on `refName` (English); no locale coupling required.

### Strings for Ranking
- Add short pill: `item.bis_rank_pill` → "BiS #{0}".
...
### Tests for Ranking
- Positive: items known to be #1/#2/#3 across defence groups return the expected rank via `getBisRank`.
...
- Eligible armour/shield items show `BiS #X` with the correct rank.
...
- Add `BIS_RANKINGS` with ordered top 3 per slot/group.
- Implement `getBisRank(item)` and integrate rank into both badges.
- Extend i18n with `item.bis_rank_pill` and optional tooltip string.
- Extend tests and snapshots to cover rank display and fallbacks.
...
- `renderer/src/web/item-check/ItemInfo.vue:4` — item header for badge.
- `renderer/src/web/price-check/filters/FilterName.vue:35` — label logic for header.
